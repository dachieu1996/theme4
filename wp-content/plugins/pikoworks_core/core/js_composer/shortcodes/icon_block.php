...
vc_map( array(
    ...
    "params"      => array( 
            ...
            array(
                    ...
                    'value' => array(
                        esc_html__('Text left', 'pikoworks_core') => 'start',
                        esc_html__('Text Center', 'pikoworks_core') => 'center',
                        esc_html__('Text Right', 'pikoworks_core') => 'end',
                        esc_html__('Text Around', 'pikoworks_core') => 'around',
                    ),
                    'std'           => 'start',
                    'admin_label' => true, 
                    'dependency' => array(
...
class WPBakeryShortCode_pikoworks_icon_block extends WPBakeryShortCode { 
    ...
    protected function content($atts, $content = null) {
        ...
        $css_class = 'icon-layout-'. $style . ' ' . $el_class;
        if ( function_exists( 'vc_shortcode_custom_css_class' ) ):
            ...
            if ( trim( $block_title ) != '' ) {
                if ( trim( $link['url'] ) != '' ) {
                    $block_title_html = '<h5><a href="' . esc_url( $link['url'] ) . '" target="' . esc_attr( $link['target'] ) . '" title="' . esc_attr( $block_title ) . '" '. $title_style .'>' . sanitize_text_field( $block_title ) . '</a></h5>';
                }
                else{
                    $block_title_html = '<h5 '.$title_style .'>' . sanitize_text_field( $block_title ) . '</h5>';   
                }
            }
            ...
            if ( trim( $block_desc ) != '' ) {
                $block_desc_html = '<div '. $text_style .'>' . sanitize_text_field( $block_desc ) .' '. $read_more_html .'</div>';
            }
           ...
           $block_class = 'align-items-center';
           $block_class2 = 'pl25';
           if($style == '2'){
               $block_class = 'flex-column align-items-'. $text_align . ' ' .$t_c.$t_r;
               $block_class2 = 'd_flex flex-column align-items-'. $text_align . ' ' .$t_c.$t_r;
           }
        
        ob_start(); 
        
        $block_html = ' <div class="d_flex icon-block '. esc_attr($block_class).'">
        <div class="icon-wrap">'. $icon_html .'</div>
         <div class="icon-content '.esc_attr($block_class2).'">
         '.$block_title_html.'         
         '. $block_desc_html .'
         </div>
        </div>';
