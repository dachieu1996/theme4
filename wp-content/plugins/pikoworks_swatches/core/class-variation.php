...
//        global $post;
    ...
    public function vs_gallery_image() { //gallery single product
		...
		?>
			...
			<?php
                        $nav_position = wooxon_get_option_data( 'optn_woo_single_products_thumbnail', 'bottom');
                        if ( $attachment_ids && $nav_position !='sticky') {
                                ?>
                                <div class="piko-nav piko-carousel oh" data-slick='{"slidesToShow": 4,"slidesToScroll": 1,"arrows": false, "focusOnSelect": true,"asNavFor": ".piko-thumb", <?php if ( $nav_position == 'left' || $nav_position == 'right' ) echo '"vertical": true,"verticalSwiping": true,'; ?> "responsive":[{"breakpoint": 991,"settings":{"slidesToShow": 3}},{"breakpoint": 576,"settings":{"slidesToShow": 4, "vertical":false,"verticalSwiping": false}}]}'>
                                        <?php
                                                foreach ( $attachment_ids as $attachment_id ) {
                                                        $image_link = wp_get_attachment_url( $attachment_id );
