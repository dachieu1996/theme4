...
if ( ! function_exists( 'wooxon_is_wc_activated' ) ) {
	function wooxon_is_wc_activated() {
		if ( class_exists( 'woocommerce' ) ) { return true; } else { return false; }
	}
}
/**
 * Query WooCommerce plugins Activation.
 * @var  $plugins main extension class name
 * @return boolean
 */
function wooxon_is_wc_plugins_activated( $plugins ) {

	if( wooxon_is_wc_activated() ) {
		$activated = class_exists( $plugins ) ? true : false;
	} else {
		$activated = false;
	}
...
if( ! function_exists( 'wooxon_is_wishlist_activated' ) ) {
        /**
	 * Check if YITH Wishlist is activated
	 */
	function wooxon_is_wishlist_activated() {
		return wooxon_is_wc_plugins_activated( 'YITH_WCWL' );
	}
}

if( ! function_exists( 'wooxon_is_compare_activated' ) ) {
        /**
	 * Check if YITH WooCompare is activated
	 */
	function wooxon_is_compare_activated() {
		return wooxon_is_wc_plugins_activated( 'YITH_Woocompare' );
	}
}

if ( ! function_exists( 'wooxon_is_dokan_activated' ) ) {
	/**
	 * Check if Dokan is activated
	 */
	function wooxon_is_dokan_activated() {
		return class_exists( 'WeDevs_Dokan' ) ? true : false;
	}
}

if ( ! function_exists( 'wooxon_is_wc_vendors_activated' ) ) {
	/**
	 * Check if WC Vendors is activated
	 */
	function wooxon_is_wc_vendors_activated() {
		return class_exists( 'WC_Vendors' ) ? true : false;
	}
}

if ( ! function_exists( 'wooxon_is_wc_marketplace_activated' ) ) {
	/**
	 * Check if WC Marketplace is activated
	 */
	function wooxon_is_wc_marketplace_activated() {
...
function wooxon_is_buddypress_activity_directory() {
...
function wooxon_is_buddypress_groups_directory() {
...
function wooxon_is_buddypress_group() {
...
function wooxon_is_buddypress_members_directory() {
...
function wooxon_is_buddypress_user() {
...
function wooxon_is_buddypress_blogs_directory() {
...
function wooxon_is_buddypress_component( $component ) {
...
// BuddyPress styles
// ------------------------------------------------------------
function wooxon_buddypress_styles() {
    $custom_skin_bg = wooxon_get_option_data('main_color_scheme', '#56cfe1'); 
