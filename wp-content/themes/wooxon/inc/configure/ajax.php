...
if(!function_exists('wooxon_variation_query')) {
    add_filter('the_posts', 'wooxon_variation_query');
    function wooxon_variation_query($posts, $query = false) {
        if ( wooxon_get_option_data('search_by_sku') ) {
            if (is_search() && !is_admin()) {
                ...
                $matchedSku = wooxon_get_parent_post_by_sku(get_search_query(), $ignoreIds);
...
if(!function_exists('wooxon_get_parent_post_by_sku')) {
    function wooxon_get_parent_post_by_sku($sku, $ignoreIds) {
        //Check for
...
add_action( 'wp_ajax_wooxon_ajax_search', 'wooxon_ajax_search_action');
add_action( 'wp_ajax_nopriv_wooxon_ajax_search', 'wooxon_ajax_search_action');
if(!function_exists('wooxon_ajax_search_action')) {
    function wooxon_ajax_search_action() {
        global $woocommerce;
        $ajax_product_count = wooxon_get_option_data( 'search_ajax_product' );
        $ajax_post_count = wooxon_get_option_data( 'search_ajax_post' );
        ...
        if(isset($_REQUEST['s'])) {
            ...
            $products = ( wooxon_get_option_data( 'search_ajax_product' ) ) ? get_posts( $args ) : '' ;
            ...
            if ( ! empty( $products) ) {
                ...
                $result['html'] .= '<h3 class="search-results-title">' . esc_html__('Products found', 'wooxon') . '<a href="' . esc_url( home_url() ) . '/?s='. $s .'&post_type=product">' . esc_html__('View all', 'wooxon' ) . '</a></h3>';
                $result['html'] .= '<ul class="ul-no">' . ob_get_clean() . '</ul>';
                $result['html'] .= '</div>';
            }
            ...
            if ( ( !empty( $regular_products ) || !empty( $variations ) ) && wooxon_get_option_data('search_by_sku') ) {

                $result['status'] = 'success';
                if ( empty($products) ) {
                    $result['html'] .= '<div class="product-piko-ajax-list product-sku-piko-ajax-list">';
                    $result['html'] .= '<h3 class="search-results-title">' . esc_html__('Products found', 'wooxon') . '<a href="' . esc_url( home_url() ) . '/?s='. $s .'&post_type=product">' . esc_html__('View all', 'wooxon' ) . '</a></h3>';
                    $result['html'] .= '<ul class="ul-no">';
                }
            ...
            if ( $_REQUEST['cat'] && ! wooxon_get_option_data( 'search_ajax_product' ) ) $args['category_name'] = $_REQUEST['cat'];

            $posts = ( wooxon_get_option_data( 'search_ajax_post' ) ) ? get_posts( $args ) : '' ;
            ...
            if ( !empty( $posts ) ) {
                ...
                $result['html'] .= '<h3 class="search-results-title">' . esc_html__('Posts found', 'wooxon') . '<a href="' . esc_url( home_url() ) . '/?s='. $s .'&post_type=post">' . esc_html__('View all', 'wooxon' ) . '</a></h3>';
                $result['html'] .= '<ul class="ul-no">' . ob_get_clean() . '</ul>';
                $result['html'] .= '</div>';
            }
            wp_reset_postdata();
            ...
            if ( empty( $products ) && empty( $posts ) && empty( $regular_products ) && empty( $variations ) ) {
                ...
                $result['html'] .= '<h3>' . esc_html__( 'Sorry, No results were found', 'wooxon' ) . '</h3>';
                $result['html'] .= '<p class="not-found-text">' .  esc_html__( 'Please try again with different keywords. Surely you can find something for yourself!', 'wooxon' ). '</p>';
                $result['html'] .= '</div>';
            }
