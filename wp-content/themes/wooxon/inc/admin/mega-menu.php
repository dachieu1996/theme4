<?php
if ( !function_exists( 'wooxon_admin_add_filter_wp_setup_nav_menu_item' ) ){
	add_filter( 'wp_setup_nav_menu_item', 'wooxon_admin_add_filter_wp_setup_nav_menu_item');
	function wooxon_admin_add_filter_wp_setup_nav_menu_item( $menu_item ){
...
if ( !function_exists( 'wooxon_admin_add_action_wp_update_nav_menu_item' ) ){
	add_action( 'wp_update_nav_menu_item', 'wooxon_admin_add_action_wp_update_nav_menu_item', 10, 3 );
	function wooxon_admin_add_action_wp_update_nav_menu_item( $menu_id, $menu_item_db_id, $args ){
		$array_objects = array(
			'icon'              => '_menu_item_icon',
			'nolink'            => '_menu_item_nolink',
			'hide'              => '_menu_item_hide',
...
if ( !function_exists( 'wooxon_admin_add_filter_wp_edit_nav_menu_walker' ) ){
	add_filter( 'wp_edit_nav_menu_walker', 'wooxon_admin_add_filter_wp_edit_nav_menu_walker', 10, 2 );
	function wooxon_admin_add_filter_wp_edit_nav_menu_walker( $walker, $menu_id ){
		if ( class_exists( 'Wooxon_Walker_Nav_Menu_Edit_Custom' ) ){
			$walker = 'Wooxon_Walker_Nav_Menu_Edit_Custom';
		}
		return $walker;
	}
}

add_filter( 'walker_nav_menu_start_el', 'wooxon_add_icon_to_menu', 10, 4 );

if( !function_exists('wooxon_add_icon_to_menu') ){
	function wooxon_add_icon_to_menu($item_output, $item, $depth, $args){
		if ( !is_a( $args->walker, 'Wooxon_Walker_Top_Nav_Menu' ) && !is_a( $args->walker, 'Wooxon_Walker_Sidebar_Nav_Menu' ) && !is_a( $args->walker, 'Wooxon_Walker_Accordion_Nav_Menu' ) && $item->icon){
...
if ( !class_exists( 'Wooxon_Walker_Nav_Menu_Edit_Custom' ) ){
	class Wooxon_Walker_Nav_Menu_Edit_Custom extends Walker_Nav_Menu  {
		/**
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ( ! empty( $item->_invalid ) ) {
				...
				$title = sprintf( esc_html__( '%s (Invalid)', 'wooxon' ), $item->title );
			} elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
				$classes[] = 'pending';
				/* translators: %s: title of menu item in draft status */
				$title = sprintf( esc_html__( '%s (Pending)', 'wooxon' ), $item->title );
			}
            ...
            <span class="item-controls">
                ...
                ?>"><?php esc_html_e( 'Edit Menu Item', 'wooxon' ); ?></a>
            </span>
					</dt>
				</dl>
				...
				<div class="menu-item-settings" id="menu-item-settings-<?php echo esc_attr($item_id); ?>">
					...
					<p class="description description-wide">
						<label for="edit-menu-item-title-<?php echo esc_attr($item_id); ?>">
							<?php esc_html_e( 'Navigation Label', 'wooxon' ); ?><br />
					...
					<p class="description">
						<label for="edit-menu-item-target-<?php echo esc_attr($item_id); ?>">
							...
							<?php esc_html_e( 'Open link in a new window/tab', 'wooxon' ); ?>
						</label>
					</p>
					<?php
					...
					<p class="description description-wide">
						<label for="edit-menu-item-icon-<?php echo esc_attr($item_id); ?>">
							<?php esc_html_e( 'Font Awesome Icon Class', 'wooxon' ); ?><br />
							...
							<span><?php echo wp_kses( __('Icon class example: fa fa-facebook. you need only put "facebook" no prefix fa fa-. You can see <a  href="https://fontawesome.com/v4.7.0/icons/" target="_blank">Font Awesome Icons in here</a>', 'wooxon'), array( 'a' => array( 'href' => array(), 'target' => array() ) ) ) ?></span>
						</label>
					</p>
					<p class="description">
						<label for="edit-menu-item-nolink-<?php echo esc_attr($item_id); ?>">
							...
							<?php esc_html_e( "Don't link", 'wooxon'); ?>
						</label>
					</p>
					<p class="description">
						<label for="edit-menu-item-hide-<?php echo esc_attr($item_id); ?>">
							...
							<?php esc_html_e( "Don't show a link", 'wooxon' ); ?>
						</label>
					</p>
					<p class="description">
						<label for="edit-menu-item-mobile_hide-<?php echo esc_attr($item_id); ?>">
							...
							<?php esc_html_e( "Don't show a link on mobile panel", 'wooxon' ); ?>
						</label>
					</p>
					<div class="edit-menu-item-level0-<?php echo esc_attr($item_id); ?>" style="<?php if ($depth == 0) echo 'display:block;'; else echo 'display:none;' ?>">
						...
						<p class="description description-thin description-thin-custom">
							<label for="edit-menu-item-type-menu-<?php echo esc_attr($item_id); ?>">
								<?php esc_html_e( 'Menu Type', 'wooxon' ); ?><br />
								<select id="edit-menu-item-type-menu-<?php echo esc_attr($item_id); ?>"
									...
									>
									<option value="" <?php if(esc_attr($item->popup_type) == ""){echo 'selected="selected"';} ?>><?php esc_html_e( 'Narrow', 'wooxon' ) ?></option>
									<option value="wide" <?php if(esc_attr($item->popup_type) == "wide"){echo 'selected="selected"';} ?>><?php esc_html_e( 'Wide', 'wooxon' ) ?></option>
								</select>
							</label>
						</p>
						<p class="description description-thin description-thin-custom">
							<label for="edit-menu-item-popup_pos-<?php echo esc_attr($item_id); ?>">
								<?php esc_html_e( 'Popup Position', 'wooxon' ); ?><br />
								<select id="edit-menu-item-popup_pos-<?php echo esc_attr($item_id); ?>"
									...
									>
									<option value="pos-left" <?php if(esc_attr($item->popup_pos) == "pos-left"){echo 'selected="selected"';} ?>><?php esc_html_e( 'Left' , 'wooxon') ?></option>
									<option value="pos-right" <?php if(esc_attr($item->popup_pos) == "pos-right"){echo 'selected="selected"';} ?>><?php esc_html_e( 'Right', 'wooxon' ) ?></option>
									<option value="" <?php if(esc_attr($item->popup_pos) == ""){echo 'selected="selected"';} ?>><?php esc_html_e( 'Justify (only wide)', 'wooxon' ) ?></option>
									<option value="pos-center" <?php if(esc_attr($item->popup_pos) == "pos-center"){echo 'selected="selected"';} ?>><?php esc_html_e( 'Center (only wide)', 'wooxon' ) ?></option>
								</select>
							</label>
						</p>
						<div style="clear:both;"></div>
						<p class="description description-wide">
							<label for="edit-menu-item-popup_cols-<?php echo esc_attr($item_id); ?>">
								...
								<select id="edit-menu-item-popup_cols-<?php echo esc_attr($item_id); ?>"
									...
									>
									<option value="" <?php if(esc_attr($item->popup_cols) == ""){echo 'selected="selected"';} ?>><?php esc_html_e( 'Select', 'wooxon' ) ?></option>
									<option value="col-4" <?php if(esc_attr($item->popup_cols) == "col-4"){echo 'selected="selected"';} ?>><?php esc_html_e( '4 Columns', 'wooxon' ) ?></option>
									<option value="col-3" <?php if(esc_attr($item->popup_cols) == "col-3"){echo 'selected="selected"';} ?>><?php esc_html_e( '3 Columns', 'wooxon' ) ?></option>
									<option value="col-2" <?php if(esc_attr($item->popup_cols) == "col-2"){echo 'selected="selected"';} ?>><?php esc_html_e( '2 Columns', 'wooxon' ) ?></option>
									<option value="col-5" <?php if(esc_attr($item->popup_cols) == "col-5"){echo 'selected="selected"';} ?>><?php esc_html_e( '5 Columns', 'wooxon' ) ?></option>
									<option value="col-6" <?php if(esc_attr($item->popup_cols) == "col-6"){echo 'selected="selected"';} ?>><?php esc_html_e( '6 Columns', 'wooxon') ?></option>
								</select>
							</label>
						</p>
						<p class="description description-wide">
							<label for="edit-menu-item-popup_max_width-<?php echo esc_attr($item_id); ?>">
								<?php esc_html_e( 'Popup Max Width (only wide)', 'wooxon' ); ?><br />
					...
					<div class="edit-menu-item-level1-<?php echo esc_attr($item_id); ?>" style="<?php if ($depth == 1) echo 'display:block;'; else echo 'display:none;' ?>">
						...
						<p class="description description-wide">
							<label for="edit-menu-item-cols-<?php echo esc_attr($item_id); ?>">
								<?php esc_html_e( 'Columns (only wide)', 'wooxon' ); ?><br />
						...
						<?php if(post_type_exists('static_block') && $posts_block = get_posts(array('posts_per_page'=>-1,'post_type'=>'static_block'))):?>
							<p class="description description-thin">
								<label for="edit-menu-item-block-<?php echo esc_attr($item_id); ?>">
									<?php esc_html_e( 'Block Name', 'wooxon' ); ?><br />
									<select class="widefat" id="edit-menu-item_block-<?php echo esc_attr($item_id); ?>"
										...
										>
										<option value="" <?php selected($item->block,"");?>><?php esc_html_e( 'Select', 'wooxon' )?></option>
					...
					<div class="edit-menu-item-level01-<?php echo esc_attr($item_id); ?>" style="<?php if ($depth == 0 || $depth == 1) echo 'display:block;'; else echo 'display:none;' ?>">
						<p class="description description-wide">
							<label for="edit-menu-item-popup_bg_image-<?php echo esc_attr($item_id); ?>">
								<?php esc_html_e( 'Background Image (only wide)', 'wooxon' ); ?><br />
						...
						<p class="description description-wide">
							<label for="edit-menu-item-popup_bg_pos-<?php echo esc_attr($item_id); ?>">
								...
								<select id="edit-menu-item-popup_bg_pos-<?php echo esc_attr($item_id); ?>"
									...
									>
									<option value="" <?php if(esc_attr($item->popup_bg_pos) == ""){echo 'selected="selected"';} ?>><?php esc_html_e( 'Select', 'wooxon' ) ?></option>
									<option value="left top" <?php if(esc_attr($item->popup_bg_pos) == "left top"){echo 'selected="selected"';} ?>><?php esc_html_e( 'Left Top', 'wooxon' ) ?></option>
									<option value="left center" <?php if(esc_attr($item->popup_bg_pos) == "left center"){echo 'selected="selected"';} ?>><?php esc_html_e( 'Left Center' , 'wooxon') ?></option>
									<option value="left center" <?php if(esc_attr($item->popup_bg_pos) == "left center"){echo 'selected="selected"';} ?>><?php esc_html_e( 'Left Center', 'wooxon' ) ?></option>
									<option value="left bottom" <?php if(esc_attr($item->popup_bg_pos) == "left bottom"){echo 'selected="selected"';} ?>><?php esc_html_e( 'Left Bottom', 'wooxon' ) ?></option>
									<option value="center top" <?php if(esc_attr($item->popup_bg_pos) == "center top"){echo 'selected="selected"';} ?>><?php esc_html_e( 'Center Top', 'wooxon' ) ?></option>
									<option value="center center" <?php if(esc_attr($item->popup_bg_pos) == "center center"){echo 'selected="selected"';} ?>><?php esc_html_e( 'Center Center', 'wooxon' ) ?></option>
									<option value="center bottom" <?php if(esc_attr($item->popup_bg_pos) == "center bottom"){echo 'selected="selected"';} ?>><?php esc_html_e( 'Center Bottom', 'wooxon' ) ?></option>
									<option value="right top" <?php if(esc_attr($item->popup_bg_pos) == "right top"){echo 'selected="selected"';} ?>><?php esc_html_e( 'Right Top', 'wooxon' ) ?></option>
									<option value="right center" <?php if(esc_attr($item->popup_bg_pos) == "right center"){echo 'selected="selected"';} ?>><?php esc_html_e( 'Right Center', 'wooxon' ) ?></option>
									<option value="right bottom" <?php if(esc_attr($item->popup_bg_pos) == "right bottom"){echo 'selected="selected"';} ?>><?php esc_html_e( 'Right Bottom', 'wooxon' ) ?></option>
									);
								</select>
							</label>
						</p>
						<p class="description description-wide">
							<label for="edit-menu-item-popup_bg_repeat-<?php echo esc_attr($item_id); ?>">
								...
								<select id="edit-menu-item-popup_bg_repeat-<?php echo esc_attr($item_id); ?>"
									...
									>
									<option value="" <?php if(esc_attr($item->popup_bg_repeat) == ""){echo 'selected="selected"';} ?>><?php esc_html_e( 'Select', 'wooxon' ) ?></option>
									<option value="no-repeat" <?php if(esc_attr($item->popup_bg_repeat) == "no-repeat"){echo 'selected="selected"';} ?>><?php esc_html_e( 'No Repeat', 'wooxon' ) ?></option>
									<option value="repeat" <?php if(esc_attr($item->popup_bg_repeat) == "repeat"){echo 'selected="selected"';} ?>><?php esc_html_e( 'Repeat All', 'wooxon' ) ?></option>
									<option value="repeat-x" <?php if(esc_attr($item->popup_bg_repeat) == "repeat-x"){echo 'selected="selected"';} ?>><?php esc_html_e( 'Repeat Horizontally', 'wooxon' ) ?></option>
									<option value="repeat-y" <?php if(esc_attr($item->popup_bg_repeat) == "repeat-y"){echo 'selected="selected"';} ?>><?php esc_html_e( 'Repeat Vertically', 'wooxon' ) ?></option>
									<option value="inherit" <?php if(esc_attr($item->popup_bg_repeat) == "inherit"){echo 'selected="selected"';} ?>><?php esc_html_e( 'Inherit', 'wooxon' ) ?></option>
								</select>
							</label>
						</p>
						<p class="description description-wide">
							<label for="edit-menu-item-popup_bg_size-<?php echo esc_attr($item_id); ?>">
								<?php esc_html_e( 'Background Size (only wide)' , 'wooxon' ); ?><br />
								<select id="edit-menu-item-popup_bg_size-<?php echo esc_attr($item_id); ?>"
									...
									>
									<option value="" <?php if(esc_attr($item->popup_bg_size) == ""){echo 'selected="selected"';} ?>><?php esc_html_e( 'Select', 'wooxon' ) ?></option>
									<option value="inherit" <?php if(esc_attr($item->popup_bg_size) == "inherit"){echo 'selected="selected"';} ?>><?php esc_html_e( 'Inherit', 'wooxon' ) ?></option>
									<option value="cover" <?php if(esc_attr($item->popup_bg_size) == "cover"){echo 'selected="selected"';} ?>><?php esc_html_e( 'Cover', 'wooxon' ) ?></option>
									<option value="contain" <?php if(esc_attr($item->popup_bg_size) == "contain"){echo 'selected="selected"';} ?>><?php esc_html_e( 'Contain', 'wooxon' ) ?></option>
								</select>
							</label>
						</p>
						<p class="description description-wide">
							<label for="edit-menu-item-popup_style-<?php echo esc_attr($item_id); ?>">
								<?php esc_html_e( 'Custom Styles (only wide)', 'wooxon' ); ?><br />
                <textarea id="edit-menu-item-popup_style-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-popup_style" rows="3" cols="20"
					...
					<p class="description description-thin">
						<label for="edit-menu-item-tip_label-<?php echo esc_attr($item_id); ?>">
							<?php esc_html_e( 'Tip Label', 'wooxon' ); ?><br />
					...
					<p class="description description-thin">
						<label for="edit-menu-item-tip_color-<?php echo esc_attr($item_id); ?>">
							<?php esc_html_e(  'Tip Text Color', 'wooxon' ); ?><br />
					...
					<p class="description description-thin">
						<label for="edit-menu-item-tip_bg-<?php echo esc_attr($item_id); ?>">
							<?php esc_html_e( 'Tip BG Color', 'wooxon' ); ?><br />
					...
					<p class="description description-wide">
						<label for="edit-menu-item-attr-title-<?php echo esc_attr($item_id); ?>">
							<?php esc_html_e( 'Title Attribute', 'wooxon' ); ?><br />
					...
					<p class="description description-thin">
						<label for="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>">
							<?php esc_html_e( 'CSS Classes (optional)', 'wooxon' ); ?><br />
					...
					<p class="description description-thin">
						<label for="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>">
							<?php esc_html_e( 'Link Relationship (XFN)', 'wooxon' ); ?><br />
					...
					<p class="description description-wide">
						<label for="edit-menu-item-description-<?php echo esc_attr($item_id); ?>">
							<?php esc_html_e( 'Description', 'wooxon' ); ?><br />
					...
					<div class="menu-item-actions description-wide submitbox">
						<?php if( 'custom' != $item->type && $original_title !== false ) : ?>
							<p class="link-to-original">
								<?php printf( esc_html__( 'Original: %s', 'wooxon' ), '<a href="' . esc_url( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
							</p>
						<?php endif; ?>
						...
						); ?>"><?php esc_html_e( 'Remove', 'wooxon' ); ?></a> <span class="meta-sep"> | </span> <a class="item-cancel submitcancel" id="cancel-<?php echo esc_attr($item_id); ?>" href="<?php echo esc_url( add_query_arg( array('edit-menu-item' => $item_id, 'cancel' => time()), esc_url( remove_query_arg( $removed_args, admin_url( 'nav-menus.php' ) ) ) ) );
						?>#menu-item-settings-<?php echo esc_attr($item_id); ?>"><?php esc_html_e( 'Cancel', 'wooxon' ); ?></a>
					</div>
...
if ( !class_exists( 'Wooxon_Walker_Top_Nav_Menu' ) ) {
	class Wooxon_Walker_Top_Nav_Menu extends Walker_Nav_Menu {
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ($item->block)
				$item_output .= '<div class="menu-block menu-block-after">'.do_shortcode('[wooxon_static_block id="'.esc_attr($item->block).'"]').'</div>';
...
if ( !class_exists( 'Wooxon_Walker_Sidebar_Nav_Menu' ) ) {
	class Wooxon_Walker_Sidebar_Nav_Menu extends Walker_Nav_Menu {
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ($item->block)
				$item_output .= '<div class="menu-block menu-block-after">'.do_shortcode('[wooxon_static_block name="'. esc_attr( $item->block ) .'"]').'</div>';
			$item_output .= $args->after;
			$args->popup_style = $popup_style;
...
if ( !class_exists( 'Wooxon_Walker_Accordion_Nav_Menu' ) ) {
	class Wooxon_Walker_Accordion_Nav_Menu extends Walker_Nav_Menu {
