...
$source = $text = $sub_title = $link = $google_fonts = $font_container = $el_id = $el_class = $css = $css_animation = $font_container_data = $google_fonts_data = $divider = $type
        = $image = $title_position = $title_font_size = $below_title = $sub_title_before = $sub_title_font_size = $sub_title_font = $sub_font_color = $sub_font_weight = $sub_font_italic = $divider_height = $secondary_btn = $hover_effect = '';
...
}elseif($divider == 'hline_inside'){
    $divider = ' pr pa_a h-line-inside ' . $divider_height;
}elseif($divider == 'hline_full'){
    $divider = ' h_line_f ';
...
$btn_before = $btn_after = $btn_title = $sec_btn_c = $btn_style = '';
...
if ( ! empty( $link ) ) {
	$link = vc_build_link( $link );
        ...
        $btn_after = '</a>';
        $sec_btn_icon = '<span class="fa fa-angle-right pl10"></span>';
        if ($secondary_btn == '2'){
            $sec_btn_c = 'btn_right';
            $sec_btn_icon = '';
        }
        
        if ($secondary_btn == '1' || $secondary_btn == '2'){            
            $secondary_btn = '<a class="h-sec-btn f_r fs15 f_w4 '.esc_attr($sec_btn_c).'" href="' . esc_url( $link['url'] ) . '"' . ( $link['target'] ? ' target="' . esc_attr( $link['target'] ) . '"' : '' ) . ( $link['rel'] ? ' rel="' . esc_attr( $link['rel'] ) . '"' : '' ) .'>'.( $link['title'] ? esc_attr( $link['title'] )  : '' ) .wp_kses_post($sec_btn_icon).'</a>';
        }
        ...
        if ( ( $sub_font_color ) != '' ) {
            $btn_style = 'style="' .  esc_attr($sub_font_color) .  '"';
        }
        if($banner_btn !== 'yes'){
            $btn_title = $extra .'<a href="' . esc_url( $link['url'] ) . '" class="banner-btn" '. $btn_style .'>'. $link['title'] .'</a>';
        }
...
if($type == 'custom_heading'){
    if ( apply_filters( 'vc_custom_heading_template_use_wrapper', false ) ) {
            ...
            $output .= $text;
            if ( $sub_title ) {
                    $output .= '<span class="sub-title '. esc_attr($sub_title_font).'" '.$style.' >' . $sub_title . '</span>';
            }
            $output .= $secondary_btn;
            $output .= '</' . $font_container_data['values']['tag'] . '>';
    }
}else{
    $output .= '<div class="block-wrap ' . esc_attr( $css_class. ' ' .$hover_effect) . '" ' . implode( ' ', $wrapper_attributes ) . '> <figure class="block-box">';
